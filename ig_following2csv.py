...
from ig.fnc import conn_graphql_following_edge, conn_restful_following_api
import argparse
import time
    ...
    try:
        for i, target in enumerate(args.target):
            if args.output is None:\
                conn_restful_following_api(user=user, cache_login=cache_login, username="".join(target))
                # conn_graphql_following_edge(user=user, username="".join(target))
            else:
                conn_restful_following_api(user=user, cache_login=cache_login, username="".join(target), output_path=args.output)
